...
layer.prompt = function(parme, yes, no){
    var log = {}, parme = parme || {}, conf = {
        ...
        dialog: {
            btns: 2,
            type: -1,
            msg: '<form method="post" enctype="multipart/form-data" action="../auth/uploadfile"><input type="'+ function(){
                if(parme.type === 1){ //密码
                    return 'password';
            ...
            }() +'" name="'+(parme.type === 2?"file":"")+'" class="xubox_prompt xubox_form" id="xubox_prompt" value="'+ (parme.val || '') +'" /></form>',
            yes: function(index){
                ...
                if(parme.callback){
                    parme.callback(log.mylocalarea);
...
layer.photos = function(options){
    ...
    var conf = {
        ...
        page: {
            ...
            log.intro = layero.find('.xubox_intro');
            log.layero = layero;
            ...
            log.event();
            if(options.afterSuccessRender){
                options.afterSuccessRender(layero,log);
            }
