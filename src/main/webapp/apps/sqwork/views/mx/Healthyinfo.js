define(['text!mx/HealthyinfoTpl.htm','BzModel','underscore','common/RespMessage'],
function(htm,BzModel,_,RespMessage){
  ...
  return {render:function(local,cb){
   var oldid=cb.params.parentForm.find('input[name=oldid]').val();
    var Healthyinfo = BzModel.extend({
    urlRoot : 'eers/healthyinfo',
     idAttribute: "hid",
    defaults: function() {
      return {
        hid:null,
  ...
  var HealthyinfoList = Backbone.Collection.extend({
    url:'eers/healthyinfo',
    model: Healthyinfo,
    //localStorage: new Backbone.LocalStorage("healthyinfo-backbone"),
    done: function() {
    ...
    var Healthyinfos = new HealthyinfoList;

  var HealthyinfoView = Backbone.View.extend({
   ...
   initialize: function() {
    ...
    // Re-render the name and tel of the healthyinfo item.
    render: function() {
  ...
  var AppView = Backbone.View.extend({
    ...
    initialize: function() {
      this.listenTo(Healthyinfos, 'add', this.addOne);
      this.listenTo(Healthyinfos, 'reset', this.addAll);
      this.listenTo(Healthyinfos, 'all', this.render);
      this.addOneView();
      var me=this;
      if(oldid){
        Healthyinfos.fetch({ data: $.param({ page: 1,customtag:'nopagination',
    ...
    addOne: function(todo) {
      var view = new HealthyinfoView({model: todo});
      var vel=view.render().el;
      this.$("div[opt=list]").append(vel);
      $.parser.parse(vel);
    },
    addOneView: function(e) {
      var m=new Healthyinfo({});
          m.on("invalid", function(model, error) {
      alert(" " + error);
    });

      var view = new HealthyinfoView({model:m});
      var vel=view.render().el;
      this.$("div[opt=list]").append(vel);
      $.parser.parse(vel);
    },
    addAll: function() {
      Healthyinfos.each(this.addOne, this);
    }
