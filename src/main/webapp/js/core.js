/* core js : jquery , bootstrap , layout */
(function (e, t) {
...
+function ($) {
    $(function () {
        ...
        $(document).on('click', '[data-toggle="divModal"]', function (e) {
            $('#divModal').remove();
            e.preventDefault();
            var $this = $(this), $target = $($this.attr('data-target') || $this.attr('href')), divhtml = $target.html();
            $target.remove();
            var $modal = $('<div class="modal fade in" id="divModal" aria-hidden="true"><div class="modal-dialog"><div class="modal-content">' + '<div class="modal-header">' + '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>' + '<h4 class="modal-title" id="divModalLabel">' + ($this.attr('data-title') == null || $this.attr('data-title') == "" ? "信息" : $this.attr('data-title')) + '</h4>' + '</div>' + '<div class="modal-body">' + divhtml + '</div>' + '<div class="modal-footer"><button id="divModalSubmit" type="button" class="btn btn-primary">' + ($this.attr('data-submit') == null || $this.attr('data-submit') == "" ? "提交" : $this.attr('data-submit')) + '</button>' + '<button type="button" class="btn btn-default" data-dismiss="modal">关&nbsp;闭</button></div>' + '</div></div></div>');
            $('body > form').append($modal);
            $modal.modal();
            if ($this.attr("data-init") != null && $this.attr("data-init") != "") {
                var fun = eval($this.attr("data-init"));
                if (typeof (fun) == "function") {
                    fun();
                }
            }
            $("#divModalSubmit").click(function () {
                var result = true;
                if ($this.attr("data-click") != null && $this.attr("data-click") != "") {
                    var fun = eval($this.attr("data-click"));
                    if (typeof (fun) == "function") result = fun();
                }
                if (result) $("#divModal").modal("hide");
            });
        });
...
!function ($) {
    $(function () {
        $("[data-rel='search']").each(function () {
            var $this = $(this);
            var $more = $('.more-search');
            $this.click(function () {
        ...
        $("[data-rel='more']").each(function () {
            var a = $(this);
            var b = $('.more');
            a.click(function () {
                if (b.length == 0) return;
                b.fadeToggle('slow', 'linear')
            })
        });
    })
}(window.jQuery);
