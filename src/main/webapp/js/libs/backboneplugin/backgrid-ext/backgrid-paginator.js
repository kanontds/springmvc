  ...
  var PageHandle = Backgrid.Extension.PageHandle = Backbone.View.extend({
    ...
    */
    render: function () {
      ...
      this.delegateEvents();
        this.$el.addClass('paginate_button');
      return this;
    },
    ...
    */
    render: function () {
      ...
      for (var i = 0; i < handles.length; i++) {
        ...
        var currentStatus=this.collection.state;
        var pg=$('<div> <div class="col-xs-6"> <div>显示 '+(currentStatus.pageSize *(currentStatus.currentPage-1)+1)+
        ' 到 '+(currentStatus.pageSize * currentStatus.currentPage)+' /总共 '+(currentStatus.totalRecords||this.collection.length)+
        ' 条记录</div> </div> ' +
        '<div class="col-xs-6" opt="ul"></div > </div> ');
        pg.find('div[opt=ul]').append(ul);
      this.$el.append(pg);
      return this;
    }
